                        ...
                        <ul>
                            ...
                            @if(Auth::guard('customer')->check())
                                <li class="header-account-dropdown">
                                    <a href="{{ route('customer.myAccount') }}" class="header-account-toggle" 
                                       style="display: flex; flex-direction: row; align-items: center; justify-content: center; line-height: 1; padding: 0 5px; position: relative; gap: 6px;">
                                        ...
                                        @endif
                                        <span class="username-text" style="font-size: 11px; font-weight: 700; color: #253D4E; text-transform: capitalize;">{{ $user->username }}</span>
                                    </a>
                                    <div class="header-account-menu">
                                        <ul>
                                            <li>
                                                <a href="{{ route('customer.myAccount') }}?tab=account-detail&edit=1">
                                                    <i class="fi-rs-user"></i> Account Details
                                                </a>
                                            </li>
                                            <li>
                                                <a href="{{ route('customer.myAccount') }}?tab=orders">
        ...
        <div class="container">
            <div class="header-wrap">
                <div class="logo logo-width-1 pt-1">
                    <a href="{{ url('/') }}"><img src="{{ asset('assets/imgs/images/chennai-angadi-logo.svg') }}"
                            alt="logo" /></a>
                </div>
                <div class="header-right">
                    ...
                    <div class="header-action-right">
                        ...
                        <img src="{{ asset('assets/imgs/images/free-delivery.svg') }}" alt="Free Delivery" style="width:40px;"/>
                        <div class="col-auto m-0 p-0 flex-row">
                            ...
                            <!-- <div class="header-action-icon-2">
                                <a href="@if(Auth::guard('customer')->check()){{ route('customer.wishlist') }}@else{{ route('login') }}@endif"
                                    class="wishlist-link">
                                    <i class="fi-rs-heart"></i>
                                    <span class="pro-count blue" id="wishlist-count">
                                        {{ auth('customer')->check() ? auth('customer')->user()->wishlist()->count() : 0 }}
                                    </span>
                                </a>
                                <a href="@if(Auth::guard('customer')->check()){{ route('customer.wishlist') }}@else{{ route('login') }}@endif"
                            ...
                            <!-- <div class="header-action-icon-2">
                                <a class="mini-cart-icon" href="{{ route('cart.page') }}">
                                    ...
                                    <span class="pro-count blue"
                                        id="header-cart-count">{{ Auth::guard('customer')->check() ? \App\Models\Cart::where('customer_id', Auth::guard('customer')->id())->count() : count(session()->get('guest_cart', [])) }}</span>
                                </a>
                            ...
                            <!-- <div class="header-action-icon-2">
                                ...
                                <a href="{{ route('customer.myAccount') }}"><span class="lable ml-0">Account</span></a>
                                <div class="cart-dropdown-wrap cart-dropdown-hm2 account-dropdown">
                                    <ul>
                                        <li>
                                            <a href="{{ route('customer.myAccount') }}"><i
        ...
        <div class="container">
            <div class="header-wrap header-space-between position-relative">
                <div class="logo logo-width-1 d-block d-lg-none">
                    <a href="{{ url('/') }}"><img src="{{ asset('assets/imgs/images/ChennaiAngadiLogo.png') }}"
                    ...
                    <div class="main-categori-wrap d-none d-lg-block">
                        ...
                        <div class="categories-dropdown-wrap categories-dropdown-active-large font-heading catfly-wrap">
                            <div class="catfly-container">
                                    ...
                                    @foreach($categories as $cat)
                                        <li class="catfly-item" data-cat-id="{{ $cat->id }}">
                                            <a href="{{ route('category.products', $cat->id) }}">
                                                ...
                                                @endphp
                                                <img src="{{ config('app.admin_asset_url') }}/{{ $mainImage }}"
                                                    alt="{{ $cat->name }}" class="catfly-img" />
                                                <span>{{ $cat->name }}</span>
                                ...
                                {{-- RIGHT: Subcategory panels (outside scrollable area) --}}
                                <div class="catfly-right">
                                    @foreach($categories as $cat)
                                        @if($cat->subcategories->count() > 0)
                                            <div class="catfly-sub-panel" data-cat-id="{{ $cat->id }}">
                                                        ...
                                                        <li class="catfly-sub-item">
                                                            <a href="{{ route('subcategory.products', $sub->id) }}">
                                                                @if($sub->subimage)
                                                                    ...
                                                                    @endphp
                                                                    <img src="{{ config('app.admin_asset_url') }}/{{ $subImage }}" alt="{{ $sub->name }}"
                                                                        ...
                                                                        <li>
                                                                            <a href="{{ route('subcategory.products', $sub->id) }}">
                                                                                @if($child->childimage)
                                                                                    ...
                                                                                    @endphp
                                                                                    <img src="{{ config('app.admin_asset_url') }}/{{ $childImage }}"
                                                                                        alt="{{ $child->name }}" class="catfly-menu-img">
                                                                                @endif
                                                                                {{ $child->name }}
                                                                            </a>
                                                                        </li>
                    ...
                    <div class="main-menu main-menu-padding-1 main-menu-lh-2 d-none d-lg-block font-heading">
                        <nav>
                            <ul>
                                ...
                                <li>
                                    <a class="{{ request()->routeIs('index') || request()->routeIs('home') || request()->is('/') ? 'active' : '' }}"
                                        href="{{ url('/') }}">Home </a>
                                </li>
                                <li>
                                    <a class="{{ request()->routeIs('pages.about') ? 'active' : '' }}"
                                ...
                                <!-- <li class="position-static mega-parent">
                                    <a class="{{ request()->routeIs('category.products') || request()->routeIs('subcategory.products') ? 'active' : '' }}"
                                ...
                                <li>
                                    <a class="{{ request()->routeIs('pages.offer') ? 'active' : '' }}"
                                        href="{{route('pages.offer')}}">Offer</a>
                                </li>

                                <li>
                                    <a class="{{ request()->routeIs('pages.contact') ? 'active' : '' }}"
                ...
                <div class="header-right-group d-none d-lg-flex" style="display: flex; align-items: center; gap: 15px;">
                    <div class="header-action-icon-2 bottom-cart-icon" style="display: flex; align-items: center;">
                        <a class="mini-cart-icon" href="{{ route('checkout-order') }}">
                            ...
                            <span class="pro-count blue"
                                id="bottom-header-cart-count">{{ Auth::guard('customer')->check() ? \App\Models\Cart::where('customer_id', Auth::guard('customer')->id())->count() : count(session()->get('guest_cart', [])) }}</span>
                        </a>
                        <a href="{{ route('checkout-order') }}"><span class="lable ms-2"> Cart</span></a>
                    </div>
                    <div class="hotline" style="display: flex; align-items: center;">
                    ...
                    <div class="header-action-2">
                        ...
                        <div class="header-action-icon-2">
                            <a class="mini-cart-icon" href="{{ route('checkout-order') }}">
                                <img alt="Cart" src="{{ asset('assets/imgs/theme/icons/icon-cart.svg') }}" />
                                <span class="pro-count blue"
                                    id="mobile-cart-count">{{ Auth::guard('customer')->check() ? \App\Models\Cart::where('customer_id', Auth::guard('customer')->id())->count() : count(session()->get('guest_cart', [])) }}</span>
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div id="mobile-header-search-bar" style="display: none; position: absolute; top: 100%; left: 0; right: 0; z-index: 9999; background: #fff; box-shadow: 0 5px 20px rgba(0,0,0,0.15); padding: 10px 15px;">
                    <form action="{{ route('search') }}" method="GET" id="mobile-header-search-form" style="position: relative;">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // (Let the form submit naturally via action="{{ route('search') }}")
        ...
        function showSuggestions(products, query) {
            ...
            products.forEach(product => {
                ...
                <a href="${product.url}" class="suggestion-item">
                    <img src="${product.image}" alt="${product.name}" 
                         onerror="this.src='{{ asset('assets/imgs/images/default-product.png') }}'" />
            ...
            const searchUrl = '{{ route("search") }}?q=' + encodeURIComponent(query);
            html += `
