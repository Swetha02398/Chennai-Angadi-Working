...
<section class="content-main">
    ...
    <div class="content-header d-flex justify-content-between align-items-center">
        ...
        <div>
            ...
            <a href="{{ route('hsncode.create') }}" class="btn btn-primary btn-sm rounded">+ Add HSN Code</a>
            @endif
        </div>
    </div>
        ...
        <header class="card-header">
            <form method="GET" action="{{ route('hsncode.table') }}" class="w-100">
                <div class="row gx-3 align-items-center">
                    ...
                    @if($search || $status)
                    <div class="col-md-2 col-6">
                        <a href="{{ route('hsncode.table') }}" class="btn btn-secondary w-100">Clear</a>
                    </div>
                    @endif
                </div>
            </form>
    ...
    <table class="table table-bordered ">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @if(auth()->user()->isSuperAdmin() || auth()->user()->hasPermission('hsncode-edit'))
                    <a href="{{ route('hsncode.edit', $hsn->id) }}"  class="btn btn-sm btn-warning"><i class="bi bi-pencil-square me-1"></i></a>
                    @endif
                    ...
                    <form action="{{ route('hsncode.delete', $hsn->id) }}" method="POST" style="display:inline;">
                        @csrf @method('DELETE')
                        <button class="btn btn-danger btn-sm" onclick="return confirm('Delete this HSN Code?')"><i class="bi bi-trash me-1"></i></button>
                    </form>
                    @endif
                </td>
            </tr>
