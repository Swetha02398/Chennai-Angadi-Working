    ...
    <style>
        ...
        <div class="pos-wrapper">
            ...
            <div class="product-panel">
                ...
                @foreach($products as $product)
                    <div class="product-row" data-product-name="{{ strtolower($product->productname) }}">
                        {{-- Product Image --}}
                        ...
                        <div class="product-info">
                            ...
                            @if($product->variants->count() == 1)
                                ...
                                @if($singleVariant && $singleVariant->quantity)
                                    <small>{{ $singleVariant->quantity->label }} (<span id="stock-display-{{ $singleVariant->id }}"
                        ...
                        @elseif($product->variants->count() == 1)
                            ...
                            <button class="btn-add" id="btn-add-{{ $variant->id }}" data-variant-id="{{ $variant->id }}"
                                data-initial-stock="{{ $variant->stock }}" onclick="addToCart(
                                                                                        ...
                                                                                        {{ $variant->id }},
                                                                                        '{{ $product->productname }}',
                                                                                        '{{ $variant->quantity ? $variant->quantity->label : '' }}',
                                                                                        {{ $variant->sell_price }},
                                                                                        {{ $product->gst ?? 0 }},
                                                                                        {{ $product->sgst ?? 0 }},
                                                                                        {{ $product->igst ?? 0 }},
                        ...
                        <div id="variants-{{ $product->id }}" style="display:none;padding-left:60px;background:#f5f5f5">
                            @foreach($product->variants as $variant)
                                <div style="display:flex;justify-content:space-between;padding:8px;border-bottom:1px solid #ddd">
                                    <div>
                                        @if($variant->quantity)
                                            <strong style="font-size:13px">{{ $variant->quantity->label }}</strong>
                                            <small style="color:#777"> (<span id="stock-display-{{ $variant->id }}" class="stock-display">Stock:
                                                    {{ $variant->stock }}</span>)</small>
                                        @endif
                                    </div>
                                    <div>
                                        <span class="product-price">₹{{ number_format($variant->sell_price, 2) }}</span>
                                        <button class="btn-add" id="btn-add-{{ $variant->id }}" data-variant-id="{{ $variant->id }}"
                                            data-initial-stock="{{ $variant->stock }}" onclick="addToCart(
                                                                                                                    {{ $product->id }},
                                                                                                                    {{ $variant->id }},
        ...
        <div class="modal" id="taxModal">
            <div class="modal-content" style="max-width:650px">
                ...
                <div class="modal-footer">
                    <button class="btn-secondary" onclick="closeTaxModal()">Cancel</button>
        ...
        <div class="modal" id="shippingAddressModal">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button class="btn-secondary" onclick="closeShippingAddressModal()">Cancel</button>
                    <button class="btn-primary" onclick="saveRegisteredShippingAddress()">Save</button>
                </div>
            </div>
        </div>
            ...
            <div class="modal-content" style="max-width:550px">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group" style="margin-bottom:15px">
                        <label style="font-weight:600;margin-bottom:5px;display:block">Apply Coupon Code</label>
                        <div style="display:flex;gap:8px">
                            <input type="text" id="couponCodeInput" placeholder="Enter coupon code" style="flex:1">
                            <button type="button" onclick="applyCoupon()"
                                style="background:#007bff;color:#fff;border:none;padding:8px 16px;border-radius:4px;cursor:pointer">Apply</button>
                        </div>
                        <div id="couponMessage" style="margin-top:5px;font-size:12px"></div>
                    </div>
                    ...
                    <div class="form-group" style="margin-bottom:12px">
                        <label style="font-weight:600;margin-bottom:5px;display:block">Billing Type</label>
                        <div style="display:flex;gap:15px">
                            <label style="display:flex;align-items:center;gap:5px;cursor:pointer">
                                <input type="radio" name="billingType" value="offline" checked> Offline
                            </label>
                            <label style="display:flex;align-items:center;gap:5px;cursor:pointer">
                                <input type="radio" name="billingType" value="online"> Online
                            </label>
                        </div>
                    </div>
                    ...
                    {{-- Payment Method --}}
                    <div class="form-group" style="margin-bottom:12px">
                        <label style="font-weight:600;margin-bottom:5px;display:block">Payment Method</label>
                    ...
                    {{-- Payment Provider --}}
                    <div class="form-group" style="margin-bottom:12px">
                        <label style="font-weight:600;margin-bottom:5px;display:block">Payment Provider</label>
                        <select id="paymentProvider" style="width:100%;padding:8px;border:1px solid #ddd;border-radius:4px">
                            <option value="cash">Cash</option>
        ...
        <script>
            ...
            function processPayment() {
                ...
                const billingType = document.querySelector('input[name="billingType"]:checked')?.value || 'offline';
