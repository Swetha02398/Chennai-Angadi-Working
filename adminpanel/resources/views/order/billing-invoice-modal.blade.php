<div class="invoice-print-content" id="billingInvoicePrintContent">
<style>
    ...
    .gi-to-label { color: #dc3545; font-weight: bold; margin-bottom: 2px; }
    .gi-customer-name { font-weight: 600; font-size: 12px; text-transform: capitalize; }
    .gi-address-line { color: #333; line-height: 1.4; font-size: 11px; }
    .gi-order-box { border-left: 1px dashed #999; padding-left: 12px; text-align: left; }
    .gi-order-id-label { font-size: 10px; color: #dc3545; font-weight: bold; }
    .gi-order-id { font-weight: bold; font-size: 12px; color: #dc3545; }
    .gi-state { font-size: 10px; color: #333; margin-top: 2px; }
    .gi-from-section { font-size: 11px; padding: 8px 0; border-bottom: 1px dashed #999; margin-bottom: 12px; }
    .gi-from-label { font-weight: bold; }
    .gi-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px; }
    .gi-logo img { height: 40px; width: auto; }
    .gi-title { text-align: center; flex: 1; }
    .gi-title h3 { font-size: 16px; font-weight: bold; margin: 0; text-transform: uppercase; }
    .gi-company { text-align: right; font-size: 10px; line-height: 1.3; }
    .gi-company-name { font-weight: 600; font-size: 11px; }
    .gi-table { width: 100%; border-collapse: collapse; margin-bottom: 0; font-size: 10px; table-layout: fixed; }
    .gi-table th { background: #d4edda; border: 1px solid #999; padding: 5px 4px; font-size: 9px; font-weight: bold; text-align: center; white-space: nowrap; }
    ...
    .gi-footer a { color: #dc3545; text-decoration: none; }
    .gst-highlight { color: #0066cc; font-weight: 600; }

    /* GST Not Included Styles (ngi- or un-prefixed mimicking billing-invoice.blade.php) */
    #billingGstNotIncludedInvoice { display: none; }
    ...
    .order-info-box { border-left: 1px dashed #999; padding-left: 15px; text-align: left; min-width: 120px; }
    ...
    .from-section { margin-bottom: 8px; font-size: 11px; padding-bottom: 6px; border-bottom: 1px dashed #999; color: #000; font-weight: normal; line-height: 1.4; }
    ...
    .items-table th { background: #d4edda; border: 1px solid #999; padding: 6px 5px; text-align: center; font-weight: bold; font-size: 11px; color: #000; }
    .items-table th:nth-child(2) { text-align: left; }
    .items-table td { padding: 5px; border: 1px solid #999; font-size: 10px; vertical-align: middle; font-weight: normal; color: #000; text-align: center; white-space: nowrap; }
    ...
    .footer { text-align: center; margin-top: 15px; font-size: 10px; color: #000; line-height: 1.4; font-weight: normal; border-top: 1px dashed #999; padding-top: 8px; }
        ...
        <tbody>
            @foreach($order->items as $index => $item)
                ...
                <tr>
                    ...
                    <td style="white-space: nowrap; text-align: right;">&#8377;{{ number_format($item->price, 2) }}</td>
                    <td>{{ $item->quantity ?? $item->qty ?? 1 }}</td>
                    <td class="subtotal-col" style="white-space: nowrap; text-align: right;">&#8377;{{ number_format($lineTotal, 2) }}</td>
                </tr>
            @endforeach
            {{-- Summary rows --}}
            @if((float)($order->discount_amount ?? 0) > 0)
            <tr>
                <td colspan="4" style="text-align: right; font-weight: bold; color: #28a745; border: 1px solid #999;">Coupon Applied @if($order->coupon_code)({{ $order->coupon_code }})@endif</td>
            ...
            <tr>
                ...
                <td style="text-align: right; font-weight: bold; color: #dc3545; border: 1px solid #999; white-space: nowrap;">&#8377;{{ number_format($order->shipping_amount, 2) }}</td>
            </tr>
            @endif
            ...
            <tr>
                ...
                <td style="text-align: right; font-weight: bold; color: #dc3545; border: 1px solid #999; white-space: nowrap;">&#8377;{{ number_format($order->cod_charge, 2) }}</td>
            </tr>
            @endif
            @endif
            <tr>
                <td colspan="4" style="text-align: right; font-weight: bold; color: #dc3545; font-size: 12px; border: 1px solid #999;">TOTAL</td>
                <td style="text-align: right; font-weight: bold; color: #dc3545; font-size: 14px; border: 1px solid #999; white-space: nowrap;">&#8377;{{ number_format(($order->final_amount ?? $order->total_amount) - ($order->tax_amount ?? 0), 2) }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <table class="gi-table">
        ...
        <tbody>
            @foreach($order->items as $index => $item)
                ...
                <tr>
                    ...
                    <td class="gst-highlight text-end">{{ $productGst }}% (&#8377;{{ number_format($gstAmount, 2) }})</td>
                    <td class="gst-highlight text-end">{{ $productSgst }}% (&#8377;{{ number_format($sgstAmount, 2) }})</td>
                    <td class="gst-highlight text-end">{{ $productIgst }}% (&#8377;{{ number_format($igstAmount, 2) }})</td>
                    <td>{{ $qty }}</td>
        ...
        <tfoot>
            ...
            <tr>
                ...
                <td style="text-align: right; font-weight: bold; font-size: 12px; color: #dc3545; border: 1px solid #999;">&#8377; {{ number_format($order->final_amount ?? $order->total_amount, 2) }}</td>
            </tr>
        </tfoot>
    </table>
