...
@section('content')
    ...
    <section class="content-main">
        <div class="content-header">
            <div>
                <h2 class="content-title">Zone States</h2>
            </div>
            <div>
        ...
        <div class="card mb-4">
            <header class="card-header">
                <form method="GET" action="{{ route('shipping.state.table') }}" class="w-100">
                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        @foreach($state as $states)
                            <tr>
                                <td>{{ ($state->currentPage() - 1) * $state->perPage() + $loop->iteration }}</td>
                                <td>{{ $states->state }}</td>
                                <td>{{ $states->zone->name ?? '-' }}</td>
                                <td>
                                    @if(auth()->user()->isSuperAdmin() || auth()->user()->hasPermission('shipping-edit'))
                                    <form action="{{ route('shipping.state.toggle', $states->id) }}" method="POST"
                                        style="display:inline-block;" onsubmit="return confirm('Change state status?');">
        ...
        <div class="pagination-area mt-30 mb-50">
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-start">
                    ...
                    @else
                        <li class="page-item">
                            <a class="page-link" href="{{ $state->appends(request()->query())->previousPageUrl() }}"
                                aria-label="Previous">
                                &laquo;
                            </a>
                        </li>
                    ...
                    {{-- Pagination Elements --}}
                    @foreach ($state->getUrlRange(1, $state->lastPage()) as $page => $url)
                        @if ($page == $state->currentPage())
                            <li class="page-item active">
                                <a class="page-link" href="#">{{ str_pad($page, 2, '0', STR_PAD_LEFT) }}</a>
                            </li>
                        @else
                            @if ($page == 1 || $page == $state->lastPage() || abs($page - $state->currentPage()) <= 1)
                                <li class="page-item">
                                    <a class="page-link"
                                        href="{{ $url }}{{ strpos($url, '?') ? '&' : '?' }}{{ http_build_query(request()->query()) }}">{{ str_pad($page, 2, '0', STR_PAD_LEFT) }}</a>
                                </li>
                            @elseif (abs($page - $state->currentPage()) == 2)
                                <li class="page-item disabled">
                    ...
                    @if ($state->hasMorePages())
                        <li class="page-item">
                            <a class="page-link" href="{{ $state->appends(request()->query())->nextPageUrl() }}"
                                aria-label="Next">
                                &raquo;
                            </a>
                        </li>
