...
<head>
    ...
    <!-- Favicon -->
    <link rel="shortcut icon" type="image/x-icon" href=" {{asset('assets/imgs/images/favicon.png') }}" />
    <!-- Template CSS -->
    <link rel="stylesheet" href=" {{asset('assets/css/plugins/animate.min.css') }}" />
    <link rel="stylesheet" href="{{ asset('assets/css/vendors/uicons-regular-straight.css') }}" />
    <link rel="stylesheet" href="{{ asset('assets/css/main.css') }}?v={{ file_exists(public_path('assets/css/main.css')) ? filemtime(public_path('assets/css/main.css')) : '6.5' }}" />
    <!-- Toastr CSS -->
...
<body>
                                    ...
                                    <figure class="quickview-img-box"
                                       >
                                        <img id="quickview-main-image"
                                            src="{{ asset('assets/imgs/theme/loading.gif') }}" alt="product image"
                ...
                <div class="mobile-sidebar-tabs">
                        ...
                        <ul class="mobile-cat-list">
                            @foreach($categories as $cat)
                                <li class="mobile-cat-item">
                                    <a href="{{ url('/' . $cat->slug) }}" class="mobile-cat-link">
                                        ...
                                        <ul class="mobile-sub-list">
                                            @foreach($cat->subcategories as $sub)
                                                <li class="mobile-sub-item">
                                                    <a href="{{ url('/' . $sub->slug) }}" class="mobile-sub-link">
                                                        @if($sub->subimage)
                                                                    ...
                                                                    <a href="{{ url('/' . $child->slug) }}">
                                                                        @if($child->childimage)
                                                                            ...
                                                                            <img src="{{ config('app.admin_asset_url') }}/{{ $childImg }}"
                                                                                alt="{{ $child->name }}" class="mobile-cat-img"
                    ...
                    <div class="mobile-tab-content" id="mob-menu">
                        <ul class="mobile-menu-list">
                            ...
                            <li><a href="{{ route('pages.about') }}"><i class="fi-rs-info"></i> About</a></li>
                            <li><a href="{{ route('pages.offer') }}"
                                    class="{{ request()->routeIs('pages.offer') ? 'active' : '' }}"><i
                                        class="fi-rs-label"></i> Offer</a></li>
                            <li><a href="{{ url('shop') }}"><i class="fi-rs-shop"></i> Shop</a></li>
                            <li><a href="{{ route('pages.contact') }}"><i class="fi-rs-headphones"></i> Contact</a></li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="mobile-header-info-wrap">                    
                    <div class="single-mobile-header-info">
                        @if(Auth::guard('customer')->check())
                            ...
                            @endphp
                            <a href="{{ route('customer.myAccount') }}" style="display: flex; flex-direction: column; align-items: center; gap: 5px; padding: 10px 0;">
                                ...
                                @endif
                                <span style="font-weight: 700; color: #253D4E; text-transform: capitalize;">{{ $user->username }}</span>
                            </a>
                        @else
                    ...
                    @if(Auth::guard('customer')->check())
                    <div class="single-mobile-header-info">
                        <a href="{{ route('customer.myAccount') }}"><i class="fi-rs-user"></i> My Dashboard </a>
                    </div>
                    @endif
                    <div class="single-mobile-header-info">
                        <a href="{{ route('customer.wishlist') }}"><i class="fi-rs-heart"></i> My Wishlist </a>
                    </div>
                    <div class="single-mobile-header-info">
                        <a href="{{ route('order.track') }}"><i class="fi-rs-marker"></i> Order Tracking </a>
                    </div>
                    <div class="single-mobile-header-info">
                        <a href="{{route('pages.contact')}}"><i class="fi-rs-envelope"></i> Contact Us </a>
                    </div>
                    <div class="single-mobile-header-info">
    ...
    <!-- Template  JS -->
    <script src="{{ asset('assets/js/main.js') }}?v={{ file_exists(public_path('assets/js/main.js')) ? filemtime(public_path('assets/js/main.js')) : '6.5' }}"></script>
    <script src="{{ asset('assets/js/shop.js') }}?v={{ file_exists(public_path('assets/js/shop.js')) ? filemtime(public_path('assets/js/shop.js')) : '6.5' }}"></script>
            ...
            document.addEventListener('click', function (e) {
                ...
                fetch(`{{ url('/product/quick-view') }}/${productId}`)
                    ...
                    .then(data => {
                        ...
                        if (imgElement) {
                            imgElement.src = "{{ asset('assets/imgs/theme/loading.gif') }}";
                            imgElement.onerror = function () {
                                this.onerror = null;
                                this.src = "{{ asset('assets/imgs/shop/product-1-1.jpg') }}";
                            };
                            const rawImage = data.raw_image || '';
                            const imageName = rawImage.split('/').pop() || rawImage.split('\\\\').pop() || rawImage;
                            const imageUrl = imageName ? '{{ config('app.admin_asset_url') }}/products/' + imageName : "{{ asset('assets/imgs/shop/product-1-1.jpg') }}";
                            imgElement.src = imageUrl;
                        }

                        document.getElementById('quickview-product-name').textContent = data.name;
                        document.getElementById('quickview-product-name').href = `{{ url('/product') }}/${data.id}`;
                        ...
                        <div class="form-group mb-2">
                            <div class="custome-checkbox">
                                ...
                                <label class="form-check-label" for="regAgree" style="font-size: 13px;">
                                    <span>I agree to terms & <a href="{{ route('pages.privacy-policy') }}"
