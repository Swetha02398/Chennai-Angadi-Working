...
@section('seo_title', $product->seo_title ?: ($product->productname . ' - ' . config('app.name')))
@section('seo_description', $product->seo_description ?: '')
@section('seo_keywords', is_array($product->seo_keywords) ? implode(', ', $product->seo_keywords) : ($product->seo_keywords ?: ''))
@section('og_title', $product->seo_title ?: ($product->productname . ' - ' . config('app.name')))
                            ...
                            <div class="product-detail accordion-detail">
                                <div class="row">
                                                ...
                                                <figure class="border-radius-10" style="position: relative;">
                                                    ...
                                                    @else
                                                        <img id="main-product-image"
                                                            ...
                                                            style="">
                                                    @endif
                                                    <div class="product-action-1" style="{{ $displayStock <= 0 ? 'display: none !important;' : '' }}">
                                                        @php
                                                            $isMainInWishlist = auth('customer')->check() && auth('customer')->user()->wishlist()->where('product_id', $product->id)->exists();
                                                        @endphp
                                                        <a aria-label="Add To Wishlist"
                                                            class="action-btn add-to-wishlist {{ $isMainInWishlist ? 'active' : '' }}"
                                                    ...
                                                    <div class="custom-thumbnails-container" id="thumbnail-slider">
                                                        @foreach($allImages as $idx => $image)
                                                            <div class="thumbnail-item {{ $idx === 0 ? 'active' : '' }}"
                                                                data-index="{{ $idx }}"
                                                                data-src="{{ env('ADMIN_ASSET_URL') }}/{{ $image }}">
                                                                <img src="{{ env('ADMIN_ASSET_URL') }}/{{ $image }}"
                                    ...
                                    <div class="col-md-8 col-sm-12 col-xs-12">
                                        <div class="detail-info pr-30 pl-30 {{ $displayStock <= 0 ? 'product-out-of-stock' : '' }}" id="main-product-info">
                                            ...
                                            {{-- WEIGHT SELECTOR - Dynamic from product_variants --}}
                                            @if($isVariableProduct && $productVariants->count() > 0)
                                                @php
                                                ...
                                                <div class="attr-detail attr-size mb-30" id="weight-selector">
                                                    ...
                                                    <select class="form-control" id="variant-dropdown"
                                                        ...
                                                        @foreach($productVariants as $index => $variant)
                                                            ...
                                                            <option value="{{ $variant->id }}" data-variant-id="{{ $variant->id }}"
                                                                data-quantity-id="{{ $variant->quantity_id }}"
                                                                data-price="{{ $displayPrice }}"
                                                                data-sell-price="{{ $variantSellPrice }}"
                                                                data-mrp-price="{{ $hasOffer ? $variantSellPrice : $variantMrp }}"
                                                                data-offer-price="{{ $hasOffer ? $variantOfferPrice : '' }}"
                                                    ...
                                                    <input type="hidden" id="selected-variant-id" name="variant_id"
                                                        value="{{ $productVariants->first()->id }}">
                                                    <input type="hidden" id="selected-quantity-id" name="quantity_id"
                                                        value="{{ $productVariants->first()->quantity_id }}">
                                                    <input type="hidden" id="selected-variant-price" name="variant_price"
                                                        value="{{ $productVariants->first()->sell_price ?? $productVariants->first()->price }}">
                                                    <input type="hidden" id="selected-weight" name="selected_weight"
                                                        value="{{ $productVariants->first()->quantity->name ?? $productVariants->first()->quantity->label ?? '' }}">
                                                    {{-- Offer discount info for JavaScript --}}
                                                    <input type="hidden" id="product-has-offer"
                                                        value="{{ $hasOffer ? '1' : '0' }}">
                                                    <input type="hidden" id="offer-discount-type"
                                                        value="{{ $discountType ?? '' }}">
                                                    <input type="hidden" id="offer-discount-value" value="{{ $discountValue }}">
                                                </div>
                                            @elseif($product->weight)
                                                {{-- Simple product with weight (no variants) --}}
                                            ...
                                            <div class="detail-extralink mb-4">
                                                <div class="detail-qty border radius">
                                                    ...
                                                    <input type="number" id="product-quantity" name="quantity"
                                                        class="qty-val" value="1" min="1" max="{{ $product->stock }}" {{ $displayStock <= 0 ? 'disabled' : '' }}>
                                                    <a href="#" class="qty-up"><i class="fi-rs-angle-small-up"></i></a>
                                                </div>
                                                <div class="product-extra-link2" style="{{ $displayStock <= 0 ? 'display: none !important;' : '' }}">
                                                    <button type="button" id="add-to-cart-btn"
                                                        class="button button-add-to-cart"
                                                        onclick="addToCart({{ $product->id }}); return false;" {{ $displayStock <= 0 ? 'disabled' : '' }}><i
                                                    ...
                                                    @endphp
                                                    <a aria-label="Add To Wishlist"
                                                        class="action-btn hover-up add-to-wishlist {{ $isInWishlist ? 'active' : '' }}"
                                                        data-id="{{ $product->id }}" href="javascript:void(0)">
                                                        <i class="fi-rs-heart"></i>
                                                    </a>
                                            ...
                                            <div class="font-lg">
                                                <!-- <ul class="mr-50 float-start">
                                                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                                                        <li class="mb-5">MFG: <span class="text-brand">{{ $product->created_at ? $product->created_at->format('M Y') : 'N/A' }}</span></li>
                                                ...
                                                <ul class="float-start">
                                                    ...
                                                    <li>Stock: <span
                                                            class="{{ $displayStock > 0 ? 'in-stock text-brand' : 'out-stock text-danger' }} ml-5"
                                                            id="product-stock-display">{{ $displayStock > 0 ? $displayStock . ' Available' : 'Out of Stock' }}</span>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                        ...
                                        <ul class="nav nav-tabs text-uppercase">
                                            ...
                                            <!-- <li class="nav-item">
                                                                                                                                                                                                                <a class="nav-link" id="Reviews-tab" data-bs-toggle="tab"
                                                                                                                                                                                                                    href="#Reviews">Reviews ({{ $product->reviews->count() }})</a>
                                                                                                                                                                                                            </li> -->
                                        </ul>
                                        <div class="tab-content shop_info_tab entry-main-content">
                                            <div class="tab-pane fade show active" id="Description">
                                                <p class="font-lg">
                                                    ...
                                                    <ul class="product-more-infor mt-30">
                                                        @if($product->taxable)
                                                            <li><span>Tax Rate</span> {{ $product->tax_rate }}%</li>
                                                        @endif
                                            ...
                                            <!-- <div class="tab-pane fade" id="Vendor-info">
                                                                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                                                                    <ul class="contact-infor mb-50">
                                                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                                                        <li><img src="{{ asset('assets/imgs/theme/icons/icon-contact.svg') }}" alt="" /><strong>Contact Seller:</strong><span>(+91) - 540-025-553</span></li>
                                                                                                                                                                                                                                                                                                    </ul>
                                                    ...
                                                    <div class="col-lg-1-5 col-md-4 col-6 col-sm-6" style="margin-bottom: 15px;">
                                                        ...
                                                        <div class="product-cart-wrap hover-up mb-30 {{ $relatedStock <= 0 ? 'product-out-of-stock' : '' }}">
                                                            <div class="product-img-action-wrap">
                                                                ...
                                                                <div class="product-action-1" style="{{ $relatedStock <= 0 ? 'display: none !important;' : '' }}">
                                                                    @php
                                                                    ...
                                                                    @endphp
                                                                    <a aria-label="Add To Wishlist"
                                                                        class="action-btn add-to-wishlist {{ $isRelatedInWishlist ? 'active' : '' }}"
                                                                        href="javascript:void(0)" data-id="{{ $related->id }}">
                                                                        <i class="fi-rs-heart"></i>
                                                                    </a>
                                                                    <a aria-label="Quick view" class="action-btn quick-view-btn"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <div class="product-rate-cover">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            <div class="product-rate d-inline-block">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                <div class="product-rating"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    style="width: {{ $relatedAvgRatingPercent }}%"></div>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            </div>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            <span
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                class="font-small ml-5 text-muted">({{ $relatedAvgRating ?: '0.0' }})</span>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        </div> -->
                                                                ...
                                                                <div class="product-card-bottom" style="flex-direction: column; align-items: stretch;">
                                                                    ...
                                                                    @if($related->variants->count() > 0)
                                                                        ...
                                                                        <div class="product-variant-selector" style="text-align: center; margin-bottom: 8px;">
                                                                            <select class="form-control product_Dropdown variant-dropdown-{{ $related->id }}"
                                                                                data-product-id="{{ $related->id }}"
                                                                                style="width: 100%; display: inline-block; appearance: none; -webkit-appearance: none; -moz-appearance: none; background-image: url('data:image/svg+xml;charset=UTF-8,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 24 24%22 fill=%22%233BB77E%22%3E%3Cpath d=%22M7 10l5 5 5-5z%22/%3E%3C/svg%3E'); background-repeat: no-repeat; background-position: right 12px center; background-size: 20px; padding-right: 40px; cursor: pointer; border: 1px solid #BCE3C9; border-radius: 5px; height: 45px; font-size: 18px;" {{ $relatedFirstVariantStock <= 0 ? 'disabled' : '' }}>
                                                                                @foreach($related->variants as $index => $variant)
                                                                                    @php
                                                                                        // Calculate offer price for this variant
                                                                                        $variantMrp = $variant->price ?? 0;
                                                                                        $variantSellPrice = $variant->sell_price ?? $variant->price;
                                                                                    ...
                                                                                    @endphp
                                                                                    <option value="{{ $variant->id }}" data-variant-id="{{ $variant->id }}"
                                                                                        data-price="{{ $displayPrice }}" data-sell-price="{{ $variantSellPrice }}"
                                                                    ...
                                                                    <div class="product-price" style="text-align: center; margin-bottom: 10px;">
                                                                        <div class="product-price-display-{{ $related->id }}" style="font-size: 15px; font-weight: 600;">
                                                                    ...
                                                                    <div
                                                                        ...
                                                                        <div style="display: flex; align-items: center;" class="col-6 pe-1">
                                                                            ...
                                                                            <input type="number" class="qty-input product-qty-{{ $related->id }}" value="1" min="1" {{ $relatedFirstVariantStock <= 0 ? 'disabled' : '' }}>
                                                                        </div>

                                                                        {{-- Add to Cart Button --}}
                                                                        <div class="add-cart col-6 ps-1" style="{{ $relatedFirstVariantStock <= 0 ? 'display: none !important;' : '' }}">
                                                                            <a href="javascript:void(0)" class="add col-12 add-to-cart-btn-{{ $related->id }}"
                                                                                data-product-id="{{ $related->id }}" data-price="{{ $relatedCurrentPrice }}"
                                                                                data-stock="{{ $relatedFirstVariantStock }}"
